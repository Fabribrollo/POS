...
export async function crearController(req: Request, res: Response): Promise<void> {
  const usuario = await usuariosService.crearUsuario(req.body as CrearUsuarioInput, req.usuario!.id);
...
export async function desactivarController(req: Request, res: Response): Promise<void> {
  const id = parseId(req.params.id);
  const usuario = await usuariosService.desactivarUsuario(id, req.usuario!.id);
  res.json(usuario);
}
